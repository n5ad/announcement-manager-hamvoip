...
FOOTER_INC="/srv/http/supermon/footer.inc"
BACKUP_SUFFIX=".bak.$(date +%Y%m%d-%H%M%S)"

if [ ! -f "$FOOTER_INC" ]; then
    echo " â†’ footer.inc not found â†’skipping"
else
    ...
    if grep -q 'include_once.*/custom/announcement\.inc' "$FOOTER_INC"; then
        echo "Announcement include already present â€” â†’skipping"
    else
...
echo_step "âœ… HamvoIP Announcement Manager Installed Successfully"
echo "73 â€” N5AD"
